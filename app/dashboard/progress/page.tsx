...
import { getMyQuizResults } from "@/lib/api/admin/quiz";
import {
  Home,
  BookOpen,
  BarChart3,
  ...
  useEffect(() => {
    const fetchProgress = async () => {
      try {
        ...
        const quizData = await getMyQuizResults();
...
function ProgressItem({
  ...
  const quiz = quizResults.find(
    (q: any) => q.lessonId === lesson._id || q.lessonId?._id === lesson._id
  );
  return (
    <div
    ...
    >
      ...
      {lesson.level && (
        ...
        <span
          className={`inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-semibold capitalize flex-shrink-0 ${LEVEL_STYLES[lesson.level] || "bg-slate-100 text-slate-600"
            }`}
        >
          {lesson.level}
        </span>
      ...
      {quiz && (
        <div className="text-sm font-semibold text-violet-600 mr-4">
          Quiz: {quiz.score}/{quiz.totalQuestions}
        </div>
      )}
